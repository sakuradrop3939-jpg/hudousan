...
BASE_URL = "https://www.athome.co.jp/kodate/chuko/{path}/list/"
...
class AthomeScraper(BaseScraper):
    ...
    def search(self, area: dict) -> list[Property]:
        results = []
        url = BASE_URL.format(path=area["athome_path"])
        page = 1
        while page <= 5:
            ...
            if len(props) < 20:
                break
            page += 1
        logger.info("アットホーム %s: %d件取得", area["name"], len(results))
        return results

    def _fetch_page(self, area: dict, url: str, page: int) -> list[Property]:
        params = {
            "max_price": "500",   # 500万円以下
            "page": page,
        }
        ...
        items = (
            soup.select("div.cassette") or
            soup.select("div[class*='object-box']") or
        ...
        for item in items:
            try:
                ...
                logger.debug("アットホーム parse error: %s", e)
        return props
    ...
    def _parse_item(self, item, area: dict, now: str) -> Property | None:
        ...
        url = f"https://www.athome.co.jp{href}" if href.startswith("/") else href

        # ── 価格 ──
        price_el = (
            item.select_one("[class*='price']") or
            item.select_one("[class*='kakaku']")
        )
