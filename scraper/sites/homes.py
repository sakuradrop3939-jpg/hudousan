...
BASE_URL = "https://www.homes.co.jp/kodate/chuko/{path}/list/"
...
class HomesScraper(BaseScraper):
    ...
    def search(self, area: dict) -> list[Property]:
        results = []
        url = BASE_URL.format(path=area["homes_path"])
        page = 1
        while page <= 5:
            props = self._fetch_page(area, url, page)
            if not props:
                break
    ...
    def _fetch_page(self, area: dict, url: str, page: int) -> list[Property]:
        params = {
            "pricemax": "5000000",
            "page": page,
        }
        ...
        items = (
            soup.select("div[class*='mod-mergeBuilding']") or
            soup.select("div[class*='imod-building']") or
            soup.select("li[class*='property']") or
            soup.select("div[class*='bukken']")
        )
        if not items:
            return []
        ...
        for item in items:
            try:
                ...
                logger.debug("ホームズ parse error: %s", e)
        return props
    ...
    def _parse_item(self, item, area: dict, now: str) -> Property | None:
        ...
        link_el = (
            ...
            item.select_one("a[href*='/kodate/']")
        )
        if not link_el:
            return None
        name = link_el.get_text(strip=True)
        href = link_el.get("href", "")
        url = f"https://www.homes.co.jp{href}" if href.startswith("/") else href

        price_el = item.select_one("[class*='price']") or item.select_one("[class*='kakaku']")
        price_text = price_el.get_text(strip=True) if price_el else ""
        price_yen, price_man = self._parse_price(price_text)
        if price_yen == 0:
            return None

        addr_el = item.select_one("[class*='address']") or item.select_one("[class*='addr']")
